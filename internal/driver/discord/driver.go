...
// DriverOption mutates Discord driver configuration.
...
// Driver adapts Discord updates into Otogi events.
type Driver struct {
	cfg     driverConfig
	source  UpdateSource
	decoder Decoder
...
// NewDriver creates a Discord driver.
func NewDriver(source UpdateSource, decoder Decoder, options ...DriverOption) (*Driver, error) {
	if source == nil {
		return nil, fmt.Errorf("new discord driver: nil source")
	}
	if decoder == nil {
		return nil, fmt.Errorf("new discord driver: nil decoder")
	}
...
// Start consumes Discord updates and publishes Otogi events.
func (d *Driver) Start(ctx context.Context, sink core.EventDispatcher) error {
	if sink == nil {
		return fmt.Errorf("start discord driver: nil sink")
	}
	...
	if err := d.source.Consume(ctx, handler); err != nil {
		...
		return fmt.Errorf("start discord driver: consume updates: %w", err)
	}
...
func (d *Driver) decodeSafely(ctx context.Context, update Update) (decoded *platform.Event, err error) {
	defer func() {
		...
		err = fmt.Errorf("decode discord update %s panic: %v", update.Type, recovered)
	}()

	decoded, err = d.decoder.Decode(ctx, update)
	if err != nil {
		return nil, fmt.Errorf("decode discord update %s: %w", update.Type, err)
	}
