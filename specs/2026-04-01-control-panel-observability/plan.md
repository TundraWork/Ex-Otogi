# Execution Plan: Control Panel Observability Foundation

## Milestone 1: Define Public Control-Panel Contracts
**Files:** `pkg/otogi/controlpanel/*`, `pkg/otogi/doc.go` if package index needs updates
...
Steps:
1. Create `pkg/otogi/controlpanel` with package docs that explain its contract-layer role.
...
3. Confirm `pkg/otogi/controlpanel` imports no `internal/*` packages and contains no goroutine or transport logic.

## Milestone 2: Implement In-Memory Sliding-Window Store
**Files:** `internal/kernel/controlpanel/*`
...
**Files:** `internal/kernel/*`, `cmd/bot/app.go`, and any required `pkg/otogi/controlpanel` context helpers
**Success criterion:** Kernel runtime wiring registers the control-panel services, and inbound handling establishes a reusable trace ID so related module/provider events can be grouped by one trace.

Steps:
1. Decide whether trace context helpers live in `pkg/otogi/controlpanel` or remain internal while preserving contract cleanliness.
...
5. Introduce control-panel configuration in `config/bot.json` parsing:
   - enable flag
   - listen address
   - bearer token
   - event window limits
...
**Success criterion:** High-value business and memory flows publish structured control-panel events in addition to existing debug logs.
...
5. Preserve current `slog` diagnostics; control-panel events are additive, not a replacement.
...
2. Confirm nil recorder or disabled control panel does not break existing module behavior.
...
Steps:
1. Resolve control-panel recorder access in provider construction or request context without violating existing boundaries.
...
Verification:
1. Add table-driven tests for successful and failed provider calls that assert control-panel event emission.
...
Steps:
1. Introduce an internal HTTP adapter package for the control panel, keeping Huma isolated from contract and kernel packages.
...
6. Update example config documentation for control-panel settings.
...
**Success criterion:** The control-panel foundation passes repository quality gates and the implemented scope matches the proposal/design/api documents.
