# Design: Control Panel Observability Foundation

## Overview

This change introduces a first-class control-panel observability plane for Ex-Otogi. The first phase focuses on read-only debugging and query workflows:
...
The control panel is therefore split into three layers:

- `pkg/otogi/controlpanel`: public contracts, DTOs, service names, and query interfaces
- `internal/kernel/controlpanel`: in-memory recorder/query implementation with sliding-window retention
- HTTP server layer: Huma-based query API adapted from the control-panel service and protected by bearer token authentication
...
### 2. `internal/kernel/controlpanel`
...
- request validation
- mapping HTTP DTOs to control-panel query DTOs
...
The control panel becomes significantly more useful once all related work for one inbound event can be grouped by `trace_id`.
...
- control panel listen address is configurable
- default listen address is `127.0.0.1`
- all HTTP endpoints require bearer token authentication
- if the control panel is enabled, token configuration is mandatory
...
The HTTP adapter is expected to use Huma as requested by the user. Huma should remain confined to the transport layer and must not leak into `pkg/otogi/controlpanel`.
