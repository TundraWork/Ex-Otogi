...
type persistedRecord struct {
	...
	Kind           ai.SemanticKind    `json:"kind,omitempty"`
	Profile        ai.SemanticProfile `json:"profile,omitempty"`
	Embedding      []float32          `json:"embedding"`
	Metadata       map[string]string  `json:"metadata,omitempty"`
	Keywords       []string           `json:"keywords,omitempty"`
	Tags           []string           `json:"tags,omitempty"`
	Links          []ai.SemanticLink  `json:"links,omitempty"`
...
func (s *Store) LoadFromFile(path string) error {
	...
	if trimmedPath == "" {
		return fmt.Errorf("semanticstore load from file: empty path")
	}

	data, err := os.ReadFile(trimmedPath)
	if err != nil {
		return fmt.Errorf("semanticstore load from file %s: %w", trimmedPath, err)
	}

	var state persistedState
	if err := json.Unmarshal(data, &state); err != nil {
		return fmt.Errorf("semanticstore load from file %s: unmarshal: %w", trimmedPath, err)
	}

	records := make(map[string]*ai.SemanticRecord, len(state.Records))
	scopes := make(map[ai.SemanticScope][]string)
	for index, item := range state.Records {
		record, err := item.toRecord()
		if err != nil {
			return fmt.Errorf("semanticstore load from file %s records[%d]: %w", trimmedPath, index, err)
		}
...
func (s *Store) SaveToFile(path string) error {
	trimmedPath := strings.TrimSpace(path)
	if trimmedPath == "" {
		return fmt.Errorf("semanticstore save to file: empty path")
	}

	state := s.snapshotState()
	payload, err := json.MarshalIndent(state, "", "  ")
	if err != nil {
		return fmt.Errorf("semanticstore save to file %s: marshal: %w", trimmedPath, err)
	}

	dir := filepath.Dir(trimmedPath)
	if err := os.MkdirAll(dir, 0o755); err != nil {
		return fmt.Errorf("semanticstore save to file %s: mkdir %s: %w", trimmedPath, dir, err)
	}

	tempFile, err := os.CreateTemp(dir, filepath.Base(trimmedPath)+".tmp-*")
	if err != nil {
		return fmt.Errorf("semanticstore save to file %s: create temp file: %w", trimmedPath, err)
	}
	tempPath := tempFile.Name()
	if _, err := tempFile.Write(payload); err != nil {
		_ = tempFile.Close()
		_ = os.Remove(tempPath)
		return fmt.Errorf("semanticstore save to file %s: write temp file: %w", trimmedPath, err)
	}
	if err := tempFile.Sync(); err != nil {
		_ = tempFile.Close()
		_ = os.Remove(tempPath)
		return fmt.Errorf("semanticstore save to file %s: sync temp file: %w", trimmedPath, err)
	}
	if err := tempFile.Close(); err != nil {
		_ = os.Remove(tempPath)
		return fmt.Errorf("semanticstore save to file %s: close temp file: %w", trimmedPath, err)
	}
	if err := os.Rename(tempPath, trimmedPath); err != nil {
		_ = os.Remove(tempPath)
		return fmt.Errorf("semanticstore save to file %s: rename temp file: %w", trimmedPath, err)
	}
...
func (r persistedRecord) toRecord() (ai.SemanticRecord, error) {
	createdAt, err := time.Parse(time.RFC3339Nano, strings.TrimSpace(r.CreatedAt))
	if err != nil {
		return ai.SemanticRecord{}, fmt.Errorf("parse created_at: %w", err)
	}
	updatedAt, err := time.Parse(time.RFC3339Nano, strings.TrimSpace(r.UpdatedAt))
	if err != nil {
		return ai.SemanticRecord{}, fmt.Errorf("parse updated_at: %w", err)
	}
...
func validatePersistedRecord(record ai.SemanticRecord) error {
