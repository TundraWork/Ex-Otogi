...
func TestStoreSaveAndLoadRoundTrip(t *testing.T) {
	...
	for _, entry := range []ai.SemanticEntry{
		{
			Scope:     scope,
	...
	if records[1].Profile.Kind != ai.SemanticKindUnit {
		t.Fatalf("profile.kind = %q, want %q", records[1].Profile.Kind, ai.SemanticKindUnit)
	}
	if records[1].Profile.AccessCount != 2 {
		t.Fatalf("profile.access_count = %d, want 2", records[1].Profile.AccessCount)
	}

	matches, err := loaded.Search(context.Background(), ai.SemanticQuery{
		Scope:         scope,
		Embedding:     []float32{1, 0},
		Limit:         5,
		MinSimilarity: 0.1,
...
func TestStoreRoundTripWithLinks(t *testing.T) {
	...
	var linkedRecord ai.SemanticRecord
