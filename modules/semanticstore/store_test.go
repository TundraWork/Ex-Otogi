...
func TestStoreStoreAndListByScope(t *testing.T) {
	...
	record, err := store.Store(context.Background(), ai.SemanticEntry{
		Scope:     testMemoryScope("chat-1"),
		Content:   "User likes tea",
		Category:  "preference",
		Embedding: []float32{1, 0},
		Metadata: map[string]string{
			ai.SemanticMetadataSource: "user",
		},
	})
	if err != nil {
	...
	if records[0].Profile.Kind != ai.SemanticKindUnit {
		t.Fatalf("profile.kind = %q, want %q", records[0].Profile.Kind, ai.SemanticKindUnit)
	}
...
func TestStoreSearchSimilarityScoring(t *testing.T) {
	...
	matches, err := store.Search(context.Background(), ai.SemanticQuery{
		Scope:         testMemoryScope("chat-1"),
		Embedding:     []float32{1, 0, 0},
		Limit:         3,
		MinSimilarity: 0.1,
...
func TestStoreSearchMinSimilarityFilteringAndLimit(t *testing.T) {
	...
	for _, embedding := range [][]float32{
		...
		if _, err := store.Store(context.Background(), ai.SemanticEntry{
			Scope:     testMemoryScope("chat-1"),
	...
	filtered, err := store.Search(context.Background(), ai.SemanticQuery{
		Scope:         testMemoryScope("chat-1"),
		Embedding:     []float32{1, 0},
		Limit:         10,
		MinSimilarity: 0.75,
	...
	limited, err := store.Search(context.Background(), ai.SemanticQuery{
		Scope:         testMemoryScope("chat-1"),
		Embedding:     []float32{1, 0},
		Limit:         1,
		MinSimilarity: 0.1,
...
func TestStoreUpdateAndDelete(t *testing.T) {
	...
	updated, err := store.Update(context.Background(), ai.SemanticUpdate{
		ID:        record.ID,
		Content:   "New memory",
		Category:  "summary",
	...
	if records[0].Profile.Kind != ai.SemanticKindSynthesized {
		t.Fatalf("profile.kind = %q, want %q", records[0].Profile.Kind, ai.SemanticKindSynthesized)
	}
...
func TestStoreNormalizesLegacyMetadataIntoProfile(t *testing.T) {
	...
	record, err := store.Store(context.Background(), ai.SemanticEntry{
		...
		Embedding: []float32{1, 0},
		Metadata: map[string]string{
			ai.SemanticMetadataImportance:       "8",
			ai.SemanticMetadataAccessCount:      "3",
			ai.SemanticMetadataLastAccessed:     time.Unix(240, 0).UTC().Format(time.RFC3339),
			ai.SemanticMetadataSource:           "natural",
			ai.SemanticMetadataSourceActorID:    "user-1",
			ai.SemanticMetadataSourceActorName:  "Alice",
			ai.SemanticMetadataSubjectActorName: "Alice",
			ai.SemanticMetadataSourceRecordIDs:  "mem-a, mem-b",
		},
	})
	if err != nil {
		t.Fatalf("Store failed: %v", err)
	...
	for worker := 0; worker < 10; worker++ {
		...
		go func(worker int) {
			...
			for index := 0; index < 20; index++ {
				...
				if _, err := store.Search(context.Background(), ai.SemanticQuery{
					Scope:         testMemoryScope("chat-1"),
					Embedding:     []float32{1, 0},
					Limit:         5,
					MinSimilarity: 0.1,
...
func TestStoreLinksRoundTrip(t *testing.T) {
	...
	record1, err := store.Store(context.Background(), ai.SemanticEntry{
		Scope:     testMemoryScope("chat-1"),
		Content:   "Alice likes tea",
		Category:  "preference",
		Embedding: []float32{1, 0},
	...
	record2, err := store.Store(context.Background(), ai.SemanticEntry{
		Scope:     testMemoryScope("chat-1"),
		Content:   "Alice likes green tea",
		Category:  "preference",
		Embedding: []float32{0.8, 0.6},
		Links: []ai.SemanticLink{
...
func TestStoreSearchExcludesExpiredRecords(t *testing.T) {
	...
	entries := []ai.SemanticEntry{
		...
		},
		{
			...
			Category:  "knowledge",
			Embedding: []float32{1, 0},
			Profile:   ai.SemanticProfile{ValidUntil: &past},
		},
		{
			Scope:     testMemoryScope("chat-1"),
			...
			Category:  "knowledge",
			Embedding: []float32{1, 0},
			Profile:   ai.SemanticProfile{ValidUntil: &future},
		},
	}
	for _, entry := range entries {
		if _, err := store.Store(context.Background(), entry); err != nil {
...
func testMemoryScope(conversationID string) ai.SemanticScope {
	return ai.SemanticScope{
		TenantID:       "tenant-1",
		Platform:       "telegram",
