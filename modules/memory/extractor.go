...
func (m *Module) processWindow(
	...
	if m == nil || m.semanticStore == nil || m.extractionProvider == nil || m.embeddingProvider == nil {
	...
	if len(lookups) > 0 && m.memory != nil {
		...
		if err != nil && m.logger != nil {
			m.logger.WarnContext(ctx, "memory resolve reply targets", "error", err)
		}
		quotes = resolved
	}
	...
	relevantExisting, err := m.semanticStore.Search(ctx, ai.SemanticQuery{
		Scope:         scope,
...
func matchRecords(matches []ai.SemanticMatch) []ai.SemanticRecord {
	records := make([]ai.SemanticRecord, len(matches))
...
func (m *Module) runExtractionLLM(
	...
	scope ai.SemanticScope,
	contextWindow extractionContext,
	existingMemories []ai.SemanticRecord,
...
func (m *Module) applyUpdate(
	...
	candidate extractedMemory,
	embedding []float32,
	relevantExisting []ai.SemanticMatch,
) error {
	...
	var existingRecord ai.SemanticRecord
	found := false
...
func (m *Module) applyDelete(
	...
	if err := m.semanticStore.Delete(ctx, targetID); err != nil {
...
func resolveSubjectActor(candidate extractedMemory, contextWindow extractionContext) *ai.SemanticActorRef {
...
func memoryActorRef(actor platform.Actor) *ai.SemanticActorRef {
