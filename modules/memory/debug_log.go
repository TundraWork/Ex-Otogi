...
func (m *Module) debugConfigLoaded(ctx context.Context, cfg Config) {
	...
	m.logger.DebugContext(ctx, "memory config loaded",
		"enabled", cfg.Enabled,
...
func (m *Module) debugWindowEnqueue(ctx context.Context, scope ai.SemanticScope, articleID string) {
	if m == nil || m.logger == nil {
		return
	}

	m.logger.DebugContext(ctx, "memory window enqueued",
...
func (m *Module) debugConsolidationStop(ctx context.Context) {
	...
	m.logger.DebugContext(ctx, "memory consolidation loop stopped")
}
...
func (m *Module) debugConsolidationCycleStart(ctx context.Context, scopeCount int) {
	...
	m.logger.DebugContext(ctx, "memory consolidation cycle",
...
func (m *Module) debugExtractionParseError(ctx context.Context, err error, response string) {
	...
	m.logger.WarnContext(ctx, "memory extraction parse failed",
		"error", err,
...
func (m *Module) debugCandidateError(ctx context.Context, candidate extractedMemory, err error) {
	...
	m.logger.WarnContext(ctx, "memory candidate processing failed",
		"error", err,
...
func (m *Module) debugConsolidationScopeStart(
	...
	m.logger.DebugContext(ctx, "memory consolidation scope prune",
		"scope_platform", scope.Platform,
		"scope_conversation_id", scope.ConversationID,
