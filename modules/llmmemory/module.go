...
func (m *Module) OnStart(ctx context.Context) error {
	...
	if strings.TrimSpace(m.cfg.PersistenceFile) != "" {
		if err := m.store.LoadFromFile(m.cfg.PersistenceFile); err != nil && !errors.Is(err, os.ErrNotExist) {
			...
			m.emitManagementEvent(ctx, "memory.store.persistence_loaded", "loaded llm memory persistence", panel.TruncateDescription(fmt.Sprintf("loaded %d records", m.store.recordCount())), panel.MemoryStoreOperationPayload{
				Store:         m.Name(),
...
func (m *Module) OnShutdown(ctx context.Context) error {
	...
	if strings.TrimSpace(m.cfg.PersistenceFile) != "" {
		...
		m.emitManagementEvent(ctx, "memory.store.persistence_saved", "saved llm memory persistence", panel.TruncateDescription(fmt.Sprintf("saved %d records", m.store.recordCount())), panel.MemoryStoreOperationPayload{
			Store:         m.Name(),
