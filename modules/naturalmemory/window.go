...
// Thread-safe.
func (m *windowManager) DrainScope(scope ai.LLMMemoryScope) (readyWindow, bool) {
	key := windowScopeKey(scope)
...
// Thread-safe.
func (m *windowManager) ActiveScopes() []ai.LLMMemoryScope {
	m.mu.Lock()
	defer m.mu.Unlock()

	scopes := make([]ai.LLMMemoryScope, 0, len(m.windows))
